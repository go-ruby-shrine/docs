...
`go-ruby-shrine/shrine` reimplements Ruby's `shrine` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-shrine/shrine` directly, with no Ruby runtime;
...
Because `shrine` is pure Go with cgo disabled, it also compiles to **WebAssembly**
